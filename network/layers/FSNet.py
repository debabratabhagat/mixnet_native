...
def switchLayer(channels, xs):
    ...
    for i in range(numofeature):
        splitxs.append(list(torch.chunk(xs[i], numofeature, dim=1)))
    ...
    for i in range(numofeature):
        ...
        for j in range(numofeature):
            if i > j:
                ...
                splitxs[j][i] = F.interpolate(splitxs[j][i], size=(h, w))
            tmp.append(splitxs[j][i])

        # Ensure all tensors in tmp have the same height and width
        min_h = min([t.shape[2] for t in tmp])
        min_w = min([t.shape[3] for t in tmp])

        for idx, t in enumerate(tmp):
            if t.shape[2] != min_h or t.shape[3] != min_w:
                tmp[idx] = F.interpolate(t, size=(min_h, min_w))

        # print(f"Tensor sizes in tmp at index {i}: {[t.shape for t in tmp]}")
...
#             if i > 0:  # Skip the first tensor since it will be used as the reference
#                 xs[i] = F.interpolate(xs[i], size=(xs[0].shape[2], xs[0].shape[3]))
#         xs[i] = torch.cat(tmp, dim = 1)
